...
let%expect_test "single buy order, nothing to match" =
  ...
  [%expect {| ACCEPTED id=1 AAPL BUY 100@$150.00 0 DAY |}]
;;
...
let%expect_test "two orders that don't cross" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL BUY 100@$150.00 0 DAY
    ACCEPTED id=2 AAPL SELL 100@$151.00 0 DAY
    |}]
;;
...
let%expect_test "exact cross at same price" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 100@$150.00 0 DAY
    ACCEPTED id=2 AAPL BUY 100@$150.00 0 DAY
...
let%expect_test "buy crosses at resting price, not aggressor price" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 100@$150.00 0 DAY
    ACCEPTED id=2 AAPL BUY 100@$151.00 0 DAY
    FILL fill_id=1 AAPL $150.00 x100 aggressor=2(Alice) BUY resting=1(Bob)
    |}]
;;
...
let%expect_test "partial fill: buy is larger than resting sell" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 60@$150.00 0 DAY
    ACCEPTED id=2 AAPL BUY 100@$150.00 0 DAY
...
let%expect_test "aggressor sweeps multiple resting orders" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 50@$150.00 0 DAY
    ACCEPTED id=2 AAPL SELL 80@$150.00 0 DAY
    ACCEPTED id=3 AAPL BUY 100@$150.00 0 DAY
...
let%expect_test "IOC: no match means immediate cancel" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL BUY 100@$150.00 0 IOC
...
let%expect_test "IOC: partial fill then cancel remainder" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 40@$150.00 0 DAY
    ACCEPTED id=2 AAPL BUY 100@$150.00 0 IOC
...
let%expect_test "orders for different symbols don't cross" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 100@$150.00 0 DAY
    ACCEPTED id=2 TSLA BUY 100@$150.00 0 DAY
    === AAPL ===
      BIDS: (empty)
      ASKS:
        $150.00 x100
...
let%expect_test "price priority: naive impl matches first-found, not best" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 100@$10.00 0 DAY
    ACCEPTED id=2 AAPL SELL 100@$10.05 0 DAY
    ACCEPTED id=3 AAPL BUY 100@$10.05 0 DAY
...
let%expect_test "scenario: two participants trade, book reflects state" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL BUY 100@$149.90 0 DAY
    REJECTED AAPL BUY 200@$149.80 reason=Duplicate ID
    ACCEPTED id=3 AAPL SELL 100@$150.10 0 DAY
    REJECTED AAPL SELL 150@$150.20 reason=Duplicate ID
    ACCEPTED id=5 AAPL BUY 50@$150.10 0 DAY
...
let%expect_test "scenario: aggressive IOC sweeps entire book" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL SELL 50@$150.00 0 DAY
    ACCEPTED id=2 AAPL SELL 50@$150.10 0 DAY
    REJECTED AAPL SELL 50@$150.20 reason=Duplicate ID
    ACCEPTED id=4 AAPL BUY 200@$150.20 0 IOC
    ...
    CANCELLED id=4 AAPL remaining=100 reason=IOC_REMAINDER
    === AAPL ===
      BIDS: (empty)
      ASKS: (empty)
      BBO: - / -
...
let%expect_test "scenario: order IDs are globally sequential" =
  ...
  [%expect
    {|
    ACCEPTED id=1 AAPL BUY 100@$150.00 0 DAY
    ACCEPTED id=2 TSLA SELL 100@$200.00 0 DAY
    ACCEPTED id=3 GOOG BUY 100@$280.00 0 DAY
    |}]
;;
...
let%expect_test "scenario: fill IDs are globally sequential" =
  ...
  [%expect
    ...
    REJECTED TSLA BUY 100@$200.00 reason=Duplicate ID
    |}]
;;
