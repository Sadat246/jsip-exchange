...
let%expect_test "e2e: two clients trade with each other" =
  with_server
    ...
    (fun ~server:_ ~port ->
      ...
      [%expect
        {| [Bob] ACCEPTED server_id=1 client_id=101 0 SELL 100@$150.00 DAY |}];
      (* Alice places a buy — should cross *)
      let%bind () = rpc_submit alice (Harness.buy ~price_cents:15000 ()) in
      [%expect
        {|
      [Alice] ACCEPTED server_id=2 client_id=102 0 BUY 100@$150.00 DAY
      [Alice] FILL fill_id=1 0 $150.00 x100 aggressor=[server_id=2 client_id=102 Alice] BUY resting=[server_id=1 client_id=101 Bob]
      [Bob] FILL fill_id=1 0 $150.00 x100 aggressor=[server_id=2 client_id=102 Alice] BUY resting=[server_id=1 client_id=101 Bob]
      |}];
      return ())
;;
...
let%expect_test "e2e: three clients, sequential orders, shared book" =
  with_server
    ...
    (fun ~server:_ ~port ->
      ...
      in
      [%expect
        {| [Bob] ACCEPTED server_id=1 client_id=101 0 SELL 50@$150.00 DAY |}];
      (* Charlie posts a sell at a higher price *)
      let%bind () =
        rpc_submit charlie (Harness.sell ~price_cents:15010 ~size:50 ())
      in
      [%expect
        {| [Charlie] ACCEPTED server_id=2 client_id=102 0 SELL 50@$150.10 DAY |}];
      ...
      [%expect
        {|
      [Alice] ACCEPTED server_id=3 client_id=103 0 BUY 80@$150.10 DAY
      [Alice] FILL fill_id=1 0 $150.00 x50 aggressor=[server_id=3 client_id=103 Alice] BUY resting=[server_id=1 client_id=101 Bob]
      [Alice] FILL fill_id=2 0 $150.10 x30 aggressor=[server_id=3 client_id=103 Alice] BUY resting=[server_id=2 client_id=102 Charlie]
      [Bob] FILL fill_id=1 0 $150.00 x50 aggressor=[server_id=3 client_id=103 Alice] BUY resting=[server_id=1 client_id=101 Bob]
      [Charlie] FILL fill_id=2 0 $150.10 x30 aggressor=[server_id=3 client_id=103 Alice] BUY resting=[server_id=2 client_id=102 Charlie]
      |}];
      (* Verify book state *)
...
let%expect_test "e2e: subscriber only sees events for subscribed symbol" =
  with_server
    ...
    (fun ~server:_ ~port ->
      ...
      in
      [%expect
        {| [Bob] ACCEPTED server_id=1 client_id=101 1 SELL 100@$200.00 DAY |}];
      (* Post on AAPL — subscriber SHOULD see this *)
      let%bind () = rpc_submit bob (Harness.sell ~price_cents:15000 ()) in
      [%expect
        {|
      [Bob] ACCEPTED server_id=2 client_id=102 0 SELL 100@$150.00 DAY
      [MD Subscriber] BBO 0 bid=- ask=$150.00 x100
      |}];
      return ())
;;
  ...
  with_server
    ...
    (fun ~server:_ ~port ->
      ...
      (* Once alice logs out, the name is available in new connections.
         ...
         without printing transient failures until the name has been
         released. *)
      let%bind () =
...
let%expect_test "e2e: cancel an order via RPC" =
  with_server
    ...
    (fun ~server:_ ~port ->
      ...
      in
      [%expect
        {| [Alice] ACCEPTED server_id=1 client_id=42 0 BUY 100@$150.00 DAY |}];
      let%bind () = rpc_cancel alice_conn cid in
      [%expect
        {| [Alice] CANCELLED server_id=1 client_id=42 0 remaining=100 reason=PARTICIPANT_REQUESTED |}];
      return ())
;;
  ...
  with_server
    ...
    (fun ~server:_ ~port ->
        ...
        Deferred.List.iter
          ...
          ~f:(fun i ->
            let%bind _ =
              rpc_submit
                seed_client
                (Harness.sell ~price_cents:(15000 + i) ())
            in
            return ())
      in
      let%bind () =
      ...
      (* Session-feed prints land on stdout in an order that depends on which
         parallel buy was processed first. Swallow the trace and assert on
         the deterministic remaining book state instead: 10 sells went in,
         the 5 buys at $150.10 each hit the lowest-priced sell, so 5 sells
         should remain. *)
...
let%expect_test "e2e: audit log subscriber sees full unfiltered stream \
  ...
  with_server
    ...
    (fun ~server:_ ~port ->
      ...
      [%expect
        {|
      [AUDIT] ACCEPTED server_id=1 client_id=101 0 SELL 100@$150.00 DAY
      ...
      [%expect
        {|
      [AUDIT] ACCEPTED server_id=2 client_id=102 1 SELL 100@$200.00 DAY
      [AUDIT] BBO 1 bid=- ask=$200.00 x100
      [Bob] ACCEPTED server_id=2 client_id=102 1 SELL 100@$200.00 DAY
      |}];
      ...
      [%expect
        {|
      [AUDIT] ACCEPTED server_id=3 client_id=103 0 BUY 100@$150.00 DAY
      [AUDIT] FILL fill_id=1 0 $150.00 x100 aggressor=[server_id=3 client_id=103 Alice] BUY resting=[server_id=1 client_id=101 Bob]
      [AUDIT] TRADE 0 $150.00 x100
      [AUDIT] BBO 0 bid=- ask=-
      [Alice] ACCEPTED server_id=3 client_id=103 0 BUY 100@$150.00 DAY
      [Alice] FILL fill_id=1 0 $150.00 x100 aggressor=[server_id=3 client_id=103 Alice] BUY resting=[server_id=1 client_id=101 Bob]
      [Bob] FILL fill_id=1 0 $150.00 x100 aggressor=[server_id=3 client_id=103 Alice] BUY resting=[server_id=1 client_id=101 Bob]
      |}];
      return ())
;;
